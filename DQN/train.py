...
from DQN import DQN_model, wrappers
import argparse
import time
import numpy as np
import collections
...
class Agent:
    ...
    def play_step(self, net, epsilon = 0.0, device="cpu"):
        ...
        if np.random.random() < epsilon:
            ...
            state_v = t.tensor(state_a).to(device)
            q_vals_v = net(state_v)
            _, act_v = t.max(q_vals_v, dim=1)
...
def calc_loss(batch, net, tgt_net, device="cpu"):
    ...
    states_v = t.tensor(states).to(device)
    next_states_v = t.tensor(next_states).to(device)
    actions_v = t.tensor(actions).to(device)
    rewards_v = t.tensor(rewards).to(device)
    done_mask = t.ByteTensor(dones).to(device)

    state_action_values = net(states_v).gather(1, actions_v.unsqueeze(-1)).squeeze(-1)  ## what this line is doing?
    ...
    while True:
        ...
        if reward is not None:
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                t.save(net.state_dict(), args.env + "-best.dat")
